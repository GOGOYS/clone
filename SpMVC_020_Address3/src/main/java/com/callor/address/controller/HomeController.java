...
//							.search(search)
//							.limit(addrService.selectAll().size())
//							.offset(0)
//							.build();
	...
	public String detail(
			...
			@PathVariable("seq") String a_seq, Model model) {
		log.debug("SEQ {}", a_seq);

		long seq = Long.valueOf(a_seq);
		AddressVO addr = addrService.findById(seq);
	 ...
	 * 	tiles폴더의 layout.xml 파일에서 해당하는 문자열로 선언된 definition을 찾는다
	 ...
	 * InternalReaiurceViewResolver가 작동되어 /views/문자열.jsp 파일을 찾아서
	 * rendering하여 응답한다.
	 */
