...
public class HomeController {
	 ...
	 * 
	 * 보통 파일 업로드를 받을때는 파일 정보를 수실할 매개변수에
	 * @RequestParam()을 사용한다
	 * 
	 * 하지만 여러파일을 수신하는 
	...
	/*
	 * @ReQuestParam() 
	 * 1. 클라이언트에서 보내는 변수와 실제 사용하는 변수의 이름이 다를 경우
	 * 2. 매개변수가 숫자형일때 클라이언트에서 변수의 값을 담지 않고 요청을 하면
	 *  내부 오류가 발생한다. 이때 required를 false로 선언하고
	 *  defaultValue를 설정하면 오류를 일부 방지할 수 있다.
	 *  
	 *  @Valid와 BindingResult result를 붙여주면
	 *  콘솔에서 오류를 볼 수 있다.
	 */
	@ResponseBody
