...
public class UserController {
	...
	public String mypage(HttpSession session, Model model) {
		 ...
		 * 왜냐, tbl_buybooks에 3개의 칼럼만 있기 때문에
		 * 이 데이터만 가지고는 구체적인 도서 정보를 알 수 없다.
		 */
		List<BuyBooksVO> buyBooks = buyService.findByUsername(loginUser.getUsername());
		
		//도서구입 리스트의 ISBN을 도서리트스에서 조회하여 가져오기
		/*
		 * 도서구입 리스트를 for()문으로 반복하면서 
		 * b_isbn 값으로 다시 도서정보를 조회한다
		 * 조회된 도서정보는 books 변수에 담는다.
		 * book 변수는 type이 BookVO 이므로 도서의 구체적인 정보가 담기게 된다.
		 */
	...
	// @ResponseBody가 있어야 return 값을 그대로 보낸다 "OK"문자열 보내기
	@ResponseBody
