document.addEventListener("DOMContentLoaded", () => {
  ...
  //div.modal-result에 포함된 요소중에서 가져와라
  ...
  modal_box.addEventListener("click",(e)=>{
  	...
  	if(td.tagName === "TD"){
  		...
  		if(codes.length > 1){
			...
			.then(res => res.json())
			.then(result =>{
				inputs[input_index.isbn].value = result.isbn.replace(extractTextPattern,"").split(" ")[1];
  ...
  btn_save?.addEventListener("click",(btn)=>{
  	...
  	// 클릭 이벤트가 발생한 tag에서 가장 가까운 form을 추출하기
